function cargarCultos() {
  ...
  contentDiv.style.position = "relative"; // to place the fixed button correctly
  ...
  // Base structure (without the button here)
  contentDiv.innerHTML = `
    <h5 style="text-align:center; font-size: 24px;">Pastor: Nélida Brito Morales Co-pastor: Marcos Rivera</h5>
    <h2 style="text-align:center; font-size: 24px;">📺 Live Streams and Previous Services</h2>
  ...
  btnVolver.addEventListener("click", () => {
    ...
    // remove fixed back button
    btnVolver.remove();
  });
  document.body.appendChild(btnVolver);

  // Show live video if available
  fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${CHANNEL_ID}&eventType=live&type=video&key=${API_KEY}`)
    ...
    .then(data => {
      ...
      if (data.items && data.items.length > 0) {
        ...
        liveContainer.innerHTML = `<p style="text-align: center; font-size: 18px; color: gray;">There is no live stream at the moment.</p>`;
      }
    });

  // Show recent videos (last 2 months)
...
`;
        ...
        pastContainer.innerHTML = `<p style="text-align: center; color: gray;">No recent videos found.</p>`;
      }
    });
}
